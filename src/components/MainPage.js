...
function MainPage() {
  ...
  const [likedBooks, setLikedBooks] = useState(false);
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        checkLikedBooks(response.data.userNum);
      })
  ...
  // 좋아요를 누른 도서가 있는지 확인하는 함수
  const checkLikedBooks = (userId) => {
    axios
      ...
      .then((response) => {
        const likedBooks = response.data.likeBookList;
        for (let book of likedBooks) {
          if (book.user_liked.includes(userId)) {
            setLikedBooks(true);
            break;
          }
        }
        ...
        >
          {likedBooks
            ...
            (<div style={{ display: 'flex' }}>
              ...
              를 기반으로 사용자에게&nbsp;
              <div> </div>
              <div style={{ fontWeight: "bold", color: "#EF9A9A" }}>
                맞춤형 도서
              </div>
              를 추천해드립니다
            </div>)
            :
            <div style={{ display: 'flex' }}>
              <div style={{ fontWeight: 'bold', color: '#EF9A9A' }}>공감 도서</div>
              를 저장하면&nbsp;
              <div style={{ fontWeight: "bold", color: "#EF9A9A" }}>지식그래프</div>
              를 기반으로 사용자에게&nbsp;
              <div style={{ fontWeight: "bold", color: "#EF9A9A" }}>
                ...
                <>
                  ...
                  >
                    ...
                    <span style={{ fontSize: "16px" }}>
                      {selectedBookReport.reportTitle}
                    </span>
                  </div>
                  <div
                    style={{
